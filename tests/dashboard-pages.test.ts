...
describe("dashboard pages", () => {
  ...
  test("renders events page and covers payload formatting", async () => {
    mocks.auth.mockResolvedValue(
      createSession({ id: "events-admin", role: "ADMIN", orgId: "org-evt" })
    );
    mocks.prisma.user.findUnique.mockResolvedValue({
      role: "ADMIN",
      orgId: "org-evt",
    ...
    mocks.prisma.user.findMany.mockResolvedValue([
      { id: "user-a", email: "member@example.com", telegramId: null },
      { id: "user-b", email: null, telegramId: "tg-200" },
    ]);
    ...
    mocks.auth.mockResolvedValue(createSession({ id: "events-user", role: "USER" }));
    mocks.prisma.user.findUnique.mockResolvedValue({
      role: "USER",
      orgId: null,
    });
    ...
    const personalHtml = render(
      await EventsPage({
        searchParams: Promise.resolve({ type: "UNKNOWN", model: "  ", limit: "5" }),
      })
    );

    expect(personalHtml).toContain("Событий пока нет.");
    expect(mocks.prisma.eventLog.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          userId: "events-user",
