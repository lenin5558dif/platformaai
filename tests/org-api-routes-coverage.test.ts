...
describe("org API routes coverage", () => {
  ...
  test("GET and POST /api/org/users cover list and invite flows", async () => {
    ...
    mocks.prisma.user.upsert.mockResolvedValueOnce({
      id: "user-3",
  ...
  test("GET /api/org/limits/summary and transfer routes cover budget and member flows", async () => {
    ...
    mocks.prisma.user.findUnique.mockResolvedValueOnce({ id: "user-1", balance: 100, costCenterId: "cc-1" });
    mocks.prisma.user.findFirst.mockResolvedValueOnce({ id: "user-2", costCenterId: "cc-2" });
    mocks.prisma.transaction.create
      .mockResolvedValueOnce({ id: "tx-1", amount: 20 })
      .mockResolvedValueOnce({ id: "tx-2", amount: 20 });
    ...
    expect(await jsonResponse(transfer)).toEqual({
      spendTx: { id: "tx-1", amount: "20" },
      refillTx: { id: "tx-2", amount: "20" },
    });

    mocks.prisma.user.findUnique.mockRejectedValueOnce(new Error("boom"));
    ...
    expect(brokenTransfer.status).toBe(500);
  });
  ...
  test("transfer route maps not found and insufficient balance errors", async () => {
    ...
    mocks.prisma.user.findUnique.mockResolvedValueOnce({ id: "user-1", balance: 5, costCenterId: null });
    const noBalance = await transferPost(
      makeRequest("http://localhost/api/org/transfer", {
        method: "POST",
        body: JSON.stringify({ userId: "user-2", amount: 20 }),
      })
    ...
    mocks.prisma.user.findUnique.mockResolvedValueOnce({ id: "user-1", balance: 100, costCenterId: null });
    mocks.prisma.user.findFirst.mockResolvedValueOnce(null);
    const missingMember = await transferPost(
      makeRequest("http://localhost/api/org/transfer", {
        method: "POST",
        body: JSON.stringify({ userId: "user-404", amount: 20 }),
      })
    );
