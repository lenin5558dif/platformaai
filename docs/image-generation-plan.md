...
## Ремонтный проход 2026-04-23

- [x] Проверить реальный каталог OpenRouter image-моделей.
- [x] Сверить прямой ответ OpenRouter с нашим парсером моделей.
- [x] Найти причину пустого списка моделей в `/api/images/models`.
- [x] Исправить парсинг `architecture.output_modalities` и `architecture.input_modalities`.
- [x] Упростить ответ `/api/images/models`, чтобы клиент не зависел от лишней вложенности.
- [x] Убрать со страницы `/images` старые объясняющие заглушки и служебные блоки.
- [x] Пересобрать пользовательский экран генерации в более минимальный сценарий: форма, результат, галерея.
- [x] Повторно прогнать локальный smoke-тест генерации и проверить выбор модели в браузере.
- [x] Прогнать unit-тесты и typecheck после фикса.

## Галерея и хранение 2026-04-23

- [x] Проверить, почему в пользовательской галерее появляются карточки без доступного файла.
- [x] Подтвердить, что проблема связана со старыми `storagePath`, ведущими на несуществующие локальные пути.
- [x] Исправить API галереи так, чтобы завершённые битые записи не попадали в пользовательский интерфейс.
...
1. Нормализуем ответ OpenRouter в `src/lib/image-models.ts`.
2. Чиним API списка моделей.
3. Делаем страницу `/images` более компактной и без лишних поясняющих блоков.
4. Проверяем сценарий генерации через браузер и тесты.
...
- [x] Добавить `/images`.
- [x] Добавить пункт в пользовательскую навигацию.
- [x] Не показывать enterprise-шум.
- [x] Добавить loading/error/success states.
- [x] Review.
- [x] Commit.
...
- [x] Добавить helper определения image-intent.
- [x] Поддержать русские и английские формулировки.
- [x] Не перехватывать обычные фразы про изображения без команды генерации.
- [x] Unit-тесты.
- [x] Review.
- [x] Commit.
...
- [x] Добавить статистику image generations в admin summary.
- [x] Показать количество генераций, ошибки, расходы.
- [x] Добавить фильтр по пользователю, если не перегружает MVP. Не добавлялся в MVP, чтобы не перегружать админку.
- [x] Тесты.
- [x] Review.
- [x] Commit.
